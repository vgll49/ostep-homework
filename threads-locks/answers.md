...
A: Command is python3 x86.py -p flag.s -t 2 -a bx=1 -M flag,count -R ax,bx -c
...
A: The flag will still be 0 in the end, the loop is just increased, so it loops 2 times for each thread. 
...
Command: python3 x86.py -p flag.s -a bx=10,bx=10 -i9 -t 2 -M flag,count -R ax,bx -c
A: Values that interrupt the flag acquisition lead to bad behaviour, because this is not an atomar operation. 
